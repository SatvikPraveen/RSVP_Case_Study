...
SELECT SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS nulls_ID, 
	   ...
	   SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS nulls_worldwide_gross_income,
...
-- The columns country_nulls, worlwide_gross_income, languages and production_company have null values.
...
-- Type your code below:
SELECT year, COUNT(id) AS number_of_movies_produced
FROM movie
...
-- There were 1007 movies produced in the year 2019 in USA or India.`
...
AS(
	SELECT production_company, 
           COUNT(m.id) as movie_count, 
           RANK() OVER(ORDER BY COUNT(m.id) DESC) as prod_camp_rank
	FROM movie m 
		 INNER JOIN ratings r ON r.movie_id = m.id
